<template>
  <div class="col-md-6 offset-md-4">
    <button type="submit" class="btn btn-primary" @click.prevent="submit">Создать мероприятие</button>
  </div>
</template>
...
export default {
  ...
  methods: {
    createFormData() {
      let form = document.querySelector('form[name="create_event"]');
    ...
    submit() {
      ...
      axios
        .post(`/admin/create_event`, this.createFormData(), {
          headers: {
        ...
        .then(data => {
          let message_arr = new Map([["ru", "Мероприятие создано"]]);
